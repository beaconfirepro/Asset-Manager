...
export async function seedDatabase(): Promise<void> {
  ...
  await db.insert(schema.syncOutboxItems).values([
    { id: id("soi"), org_id: ORG, entity_type: "inspection_schedule", entity_id: schedDraft, operation: "CREATE", payload: JSON.stringify({ schedule_id: schedDraft, hubspot_asset_id: "hs_asset_001" }), target_provider: "HUBSPOT", client_uuid: id("cu"), status: "PENDING", attempts: 0, last_attempt_at: null, error: null, created_at: now, updated_at: now },
    { id: id("soi"), org_id: ORG, entity_type: "asset_compliance_link", entity_id: aclNonCompliant, operation: "UPDATE", payload: JSON.stringify({ compliance_status: "NON_COMPLIANT" }), target_provider: "HUBSPOT", client_uuid: id("cu"), status: "IN_FLIGHT", attempts: 1, last_attempt_at: nowIso(), error: null, created_at: now, updated_at: now },
    { id: id("soi"), org_id: ORG, entity_type: "inspection_result", entity_id: resultCompleted, operation: "UPDATE", payload: JSON.stringify({ status: "COMPLETED" }), target_provider: "HUBSPOT", client_uuid: id("cu"), status: "SYNCED", attempts: 1, last_attempt_at: daysAgo(28), error: null, created_at: now, updated_at: now },
    { id: id("soi"), org_id: ORG, entity_type: "maintenance_record", entity_id: id("mr"), operation: "CREATE", payload: JSON.stringify({ maintenance_type: "CORRECTIVE" }), target_provider: "CONNECTEAM", client_uuid: id("cu"), status: "FAILED", attempts: 5, last_attempt_at: daysAgo(1), error: "ConnecteamConnector: connection refused (INACTIVE)", created_at: now, updated_at: now },
    { id: id("soi"), org_id: ORG, entity_type: "invoice", entity_id: rptSent, operation: "CREATE", payload: JSON.stringify({ report_id: rptSent, total: 1800.00 }), target_provider: "QBO", client_uuid: id("cu"), status: "CONFLICT", attempts: 3, last_attempt_at: daysAgo(2), error: "QBO: duplicate invoice number detected", created_at: now, updated_at: now },
    { id: id("soi"), org_id: ORG, entity_type: "inspection_result", entity_id: resultVoid, operation: "DELETE", payload: JSON.stringify({ reason: "customer_cancelled" }), target_provider: "HUBSPOT", client_uuid: id("cu"), status: "SYNCED", attempts: 1, last_attempt_at: daysAgo(90), error: null, created_at: now, updated_at: now },
  ]);
}
